...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(`${num}`.split('').reduce((acc, el) => acc + +el, 0));
}
...
function isBracketsBalanced(str) {
  ...
  const reg = /\(\)|\[\]|{}|<>/;
  let brackets = str;
  while (reg.test(brackets)) {
    brackets = brackets.replace(reg, '');
  }
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < position.length; i += 1) {
    if (position[i].length === 3 && position[i][i] !== undefined) {
